$(function () {
    ...
    // auto load more feed after scroller is -100px from bottom
    ...
    $(window).scroll(function () {
        if ($(window).scrollTop() >= $(document).height() - $(window).height() - 100) {
            if (!isScrolled) {
                isScrolled = true;
                var feedList = $('#feeds');
                var activeGame = $('#activeGameName').attr('class').substring(4);
                var category = $("#gameCategories li.active a").html().toLowerCase();
                var page = parseInt(feedList.attr('data-page')) + 1;
                var index = parseInt(feedList.attr('data-index') + 1);

                feedList.toggleLoadingImage();
                $.ajax({
                    url: baseUrl + '/game/' + activeGame + '/' + category + '/' + page + '/' + index,
                    timeout: 5000
                }).success(function (data) {
                        isScrolled = false;
                        feedList.append(data).toggleLoadingImage();
                        console.log(data);
                    });
                feedList.attr('data-index', index);
                feedList.attr('data-page', page);
            }
        }
    });
